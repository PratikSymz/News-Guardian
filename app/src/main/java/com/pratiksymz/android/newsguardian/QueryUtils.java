...
public final class QueryUtils {
    ...
    /**
     * Returns {@link List <NewsClass>} from the given string URL.
     */
    public static List<NewsClass> fetchNewsData(String requestURL, Context context) {
        // Create Url object
        ...
        List<NewsClass> newsItems = extractNewsData(jsonResponse, context);
    ...
    public static String makeHTTPRequest(URL url, Context context) throws IOException {
        ...
        try {
            ...
            handler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(mContext, "Problem retrieving the news results", Toast
                            .LENGTH_SHORT)
                            .show();
                }
            });
    ...
    public static List<NewsClass> extractNewsData(String jsonResponse, Context context) {
        final Context mContext = context;
        ...
        List<NewsClass> newsItems = new ArrayList<>();
        ...
        try {
            ...
            for (int i = 0; i < newsResults.length(); i++) {
                ...
                NewsClass newsObject = new NewsClass(title, section, webUrl, publicationDate);
                newsItems.add(newsObject);
            }
            ...
            handler.post(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(mContext, "Problem parsing the news results", Toast
                            .LENGTH_SHORT)
                            .show();
                }
            });
